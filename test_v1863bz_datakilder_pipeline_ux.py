...
def test_kilder_og_import_replaces_visible_pipeline_workspace():
    ...
    assert '("Kilder og import", render_analysis_pipeline_control_center_v1863bv)' in app
...
def test_source_buttons_open_ai_candidate_source_panels():
    app = (ROOT / "app.py").read_text(encoding="utf-8")
    ...
    assert 'Åpne radar/kildetest' in app
    assert '_pipeline_go_to_panel_v1863by("AI Kandidattest", "Finansavisen Bjellesauer")' in app
    assert '_pipeline_go_to_panel_v1863by("AI Kandidattest", "Oljefond Radar")' in app
    assert '_pipeline_go_to_panel_v1863by("AI Kandidattest", "Folketrygdfondet")' in app
...
def test_folketrygdfondet_has_import_export_and_ai_candidate_actions():
...
def test_version_records_ai_candidate_source_hub():
    version = (ROOT / "app_version.py").read_text(encoding="utf-8")

    assert 'APP_VERSION = "v18.6.4o"' in version
    assert "AI Candidate Source Hub" in version
    assert "Kilder og import erstatter Dataunderlag" in version
