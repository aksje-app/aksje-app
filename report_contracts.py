...
def _candidate_decisions(rows: Sequence[Mapping[str, Any]], contracts: Sequence[Mapping[str, Any]] | None = None) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, 1):
        ...
        decision = CandidateDecision(
            ...
            market=str(row.get("market") or ""),
            score=score,
            action=str(contract.get("action") or row.get("autonomy_outcome_code") or row.get("portfolio_action") or row.get("status") or "REVIEW"),
            status=str(contract.get("action_label") or row.get("autonomy_outcome_label") or row.get("status") or ""),
...
def build_report_document(run: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    sections = (
        ...
        _section("candidate_data_audit", "Kandidatdekning og redningskø", dict(decision_report.get("candidate_data_audit") or {}), 19),
        _section("short_intelligence", "Shortinteresse og shortdekning", dict(decision_report.get("short_intelligence") or {}), 19, technical=True),
        ...
        _section("learning_portfolio", "Kanonisk læringsportefølje", {
            ...
            "account": dict(((run.get("autonomous_chain") or {}).get("autonomy_learning_account") or {})),
        }, 59),
