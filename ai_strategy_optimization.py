...
def render_ai_strategy_optimization_tab() -> None:
            ...
            for proposal in proposals:
                ...
                with st.expander(title, expanded=False):
                    ...
                    if c.button("Rollback markering", key=f"rollback_{proposal['proposal_id']}"):
                        ...
                        st.rerun()
            st.info("Godkjenning er kun en revidert beslutning. Aktiv handelskonfigurasjon endres ikke automatisk.")
    ...
    with tabs[3]:
        ...
        if audit:
            st.dataframe(pd.DataFrame(audit) if pd is not None else audit, width="stretch", hide_index=True)
        else:
