...
def candidate_action_payload(item: Mapping[str, Any] | None, decision: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "decision_diff": dict(row.get("decision_diff") or dec.get("decision_diff") or {}),
        "counter_hypothesis": dict(row.get("counter_hypothesis") or dec.get("counter_hypothesis") or {}),
        "critical_assumptions": list(row.get("critical_assumptions") or dec.get("critical_assumptions") or []),
        "decision_contract": dict(row.get("decision_contract") or dec.get("decision_contract") or {}),
