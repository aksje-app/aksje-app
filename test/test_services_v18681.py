...
from core_architecture import AppStateStore
...
class ServicesV18681Tests(unittest.TestCase):
    ...
    def test_registry_contains_consolidated_services(self):
        registry = build_service_registry(session_state={})
        self.assertIsNotNone(registry.app_state)
        self.assertIsNotNone(registry.currency)
        self.assertIsNotNone(registry.notifications)
        self.assertIsNotNone(registry.review_queue)
        self.assertIsNotNone(registry.trading_rules)
